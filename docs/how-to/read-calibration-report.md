...
using the measurement's saved parameter revision or exact candidate, setup, complete subject and
...
The measurement must use a saved revision without overrides or an exact retained
candidate and belong to the displayed sample revision. Unsaved configuration
sources cannot supply this context. Candidate inputs are labeled with their proposal
and source run; they do not inherit saved-revision applicability. Selecting a different measurement clears
...
restore. Current schema 98 retains their storage; use a fresh development data directory and
