...
The current runtime uses project store **73**, including immutable author source
...
development schemas 68, 69, 70, 71 and 72 can be upgraded through an [explicit copy](migrate-data.md). Version 67 and
older stores require their pinned matching readers. No implicit migration occurs.

Within version 73, snapshots retain author source bundles, active revision,
...
Snapshot format 1 can verify and restore the tested development schemas 68, 69, 70, 71, 72 and
73. Restore retains its original schema; the current runtime still accepts only
73. Use [migrate a data copy](migrate-data.md) for the explicit 68→69→70→71→72→73 upgrade
