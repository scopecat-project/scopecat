...
class Worker:
    def __init__(self, _root: Path, revision: str, _module: str, **_: object) -> None:
...
def workers() -> Iterator[list[Worker]]:
    ...
    def spawn(root: Path, revision: str, module: str, **kwargs: object) -> Worker:
        worker = Worker(root, revision, module, **kwargs)
...
def test_warm_revision_progresses_while_same_revision_waits(
    ...
    pool.call(tmp_path, request("a"))
    pool.call(tmp_path, request("b"))
    a, b = workers
    try:
        with ThreadPoolExecutor(max_workers=2) as executor:
            active = executor.submit(pool.call, tmp_path, request("a", slow=True))
            try:
                assert a.entered.wait(2)
                independent = executor.submit(pool.call, tmp_path, request("b"))
                assert independent.result(timeout=2).stdout == b.revision
                with pytest.raises(
                    subprocess.TimeoutExpired, match="author worker queue"
                ):
                    pool.call(tmp_path, request("a"), timeout=0.01)
                assert len(workers) == 2
                assert not a.closed
            finally:
                a.release.set()
            assert active.result(timeout=2).stdout == a.revision
        assert pool.call(tmp_path, request("a")).stdout == a.revision
    finally:
        pool.close()
...
def test_new_revision_waits_for_idle_slot_and_never_evicts_active(
    ...
    try:
        with ThreadPoolExecutor(max_workers=3) as executor:
            first = executor.submit(pool.call, tmp_path, request("a", slow=True))
            second = executor.submit(pool.call, tmp_path, request("b", slow=True))
            try:
                assert a.entered.wait(2) and b.entered.wait(2)
                with pytest.raises(
                    subprocess.TimeoutExpired, match="author worker queue"
                ):
                    pool.call(tmp_path, request("c"), timeout=0.01)
                assert len(workers) == 2
                third = executor.submit(pool.call, tmp_path, request("c"))
                b.release.set()
    ...
    try:
        with ThreadPoolExecutor(max_workers=2) as executor:
            ...
            try:
                ...
                b.release.set()
                second.result(timeout=2)
                pool.publish_validated(tmp_path, tmp_path, ref, publish)
                assert published.is_set() and b.closed and not a.closed
                assert pool.call(tmp_path, request("c")).stdout == ref.content_hash
            finally:
                a.release.set()
                b.release.set()
            first.result(timeout=2)
...
def test_publication_preserves_equivalent_active_worker(
    ...
    try:
        with ThreadPoolExecutor(max_workers=1) as executor:
            active = executor.submit(pool.call, tmp_path, command)
            try:
                assert a.entered.wait(2)
                pool.publish_validated(
                    tmp_path, tmp_path, command.code_revision, AuthorRevisionState
                )
