...
class ExperimentContext:
    ...
    def coordinate[T](self, value: Input[T], /) -> CoordinateRef[T]:
        ...
        Use for scannable ControlSpec inputs or coordinates returned by scan().
        This does not create an axis, change traversal, or record an extra result.
        Return the handle to include the coordinate in the normal result tree.
        """
        if not isinstance(value, CoordinateRef):
            raise TypeError(
                "coordinate() requires a direct point coordinate; declare an "
                "Input with ControlSpec(scannable=True) or use the coordinate "
                "returned by scan(). "
